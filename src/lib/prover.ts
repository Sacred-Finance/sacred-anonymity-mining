...
import type { Circuit } from '@unirep/circuits'
...
export default {
  ...
  verifyProof: async (
    ...
    console.log(
      `Verifying proof for ${circuitName} -- checking if vkey is cached`
    )
    if (!this.cachedVkeys[circuitName]) {
      console.log(
        `Verifying proof for ${circuitName} -- fetching vkey from server`
      )
      this.cachedVkeys[circuitName] = await fetch(
        `/circuits/${circuitName}.vkey.json`
      )
    }
    console.log(
      `Verifying proof for ${circuitName} -- vkey cached, verifying proof`
    )
