...
export const isAuthValid = async request => {
  ...
  if (request.nextUrl.pathname.includes('admin')) {
    ...
    // get chainId
    const chainId = request.nextUrl.pathname.split('/')[3]
    ...
    const isModeratorResponse = await callContract(
      provider,
      address,
      'isModerator'
    )
    ...
    if (
      isAdminResponse.status === 'error' ||
      isModeratorResponse.status === 'error'
    ) {
      // Handle error
      console.log(
        'Error while checking roles:',
        isAdminResponse.error,
        isModeratorResponse.error
      )
      return false
    }
