...
export default async (req, res) => {
    ...
    try {
        ...
        rawCommunitiesData = await Promise.all(groups.map(groupId => forumContract.groupAt(groupId)));
    } catch (e) {
    ...
    try {
        communitiesData = await Promise.all(rawCommunitiesData.filter(r => !r.removed).map(rawGroupData => augmentGroupData(rawGroupData)));
    } catch (e) {
