...
const VoteButton = ({ icon, ariaLabel, voteCount, isLoading, ...props }) => {
  return (
      ...
      >
        { isLoading  ? <span className='text-primary-500'><CircularLoader className={'h-6 w-6'}/></span> : icon}
        <span className="text-sm">{voteCount}</span>
      </button>
  );
}

export const VoteUpButton = (props) => <VoteButton icon={<UpVoteIcon fill={'#4CAF50'} stroke={'#4CAF50'} />} ariaLabel="upvote" {...props} />;
export const VoteDownButton = (props) => <VoteButton icon={<DownVoteIcon fill={'#4CAF50'} stroke={'#4CAF50'} />} ariaLabel="downvote" {...props} />;
