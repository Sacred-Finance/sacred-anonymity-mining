...
const TimeSinceTopicCreated: React.FC<{ topic: Topic }> = ({ topic }) => (
  <li className="my-1">Created: {formatDistanceToNow(new Date(topic.created_at))}</li>
)
  ...
  return (
    <Link
      ...
      className={clsx(
        ...
        variant === 'banner'
          ? 'pointer-events-auto '
          : 'hover-peer:-z-[1] peer overflow-hidden border border-gray-900 bg-white transition-all duration-300 ease-in-out hover:z-[150] sm:w-full md:w-auto dark:bg-gray-900'
      )}
    >
